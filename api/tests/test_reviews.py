...
def test_create_review_menu_item_not_found(
    ...
    assert exception.value.detail == "MenuItem not found"
...
def test_read_all_reviews_success(
    db_session,
    mocker
):
    review_one = mocker.Mock()
    review_one.reviewID = 1

    review_two = mocker.Mock()
    review_two.reviewID = 2
...
def test_read_all_reviews_empty(
    db_session
):
    query = db_session.query.return_value
    ordered_query = query.order_by.return_value
...
def test_read_all_reviews_database_error(
    db_session
):
...
def test_read_one_review_success(
    db_session,
    mocker
):
    expected_review = mocker.Mock()
    expected_review.reviewID = 3
    ...
    assert result.reviewID == 3
    assert result.rating == 4


def test_read_one_review_not_found(
    db_session
):
    query = db_session.query.return_value
    filtered_query = query.filter.return_value
...
def test_read_reviews_by_menu_item_success(
    ...
    review_one = mocker.Mock()
    review_one.reviewID = 1
    review_one.item_id = 2

    review_two = mocker.Mock()
    review_two.reviewID = 2
    review_two.item_id = 2
    ...
    menu_item_query.filter.assert_called_once()
    reviews_query.filter.assert_called_once()
    filtered_reviews.order_by.assert_called_once()
    ordered_reviews.all.assert_called_once()
...
def test_read_reviews_by_menu_item_database_error(
    ...
    assert exception.value.detail == (
        "Unable to read menu item reviews"
    )
...
def test_update_review_success(
    ...
    existing_review.reviewID = 1
    existing_review.rating = 3

    updated_review = mocker.Mock()
    updated_review.reviewID = 1
    ...
    query = db_session.query.return_value
    review_query = query.filter.return_value

    review_query.first.side_effect = [
        existing_review,
        updated_review
    ]
    ...
    review_query.update.assert_called_once_with(
        {
            "rating": 5,
            "comment": "Updated review"
        },
...
def test_update_review_not_found(
    ...
    review_query.update.assert_not_called()
    db_session.commit.assert_not_called()


def test_update_review_database_error(
    db_session,
    mocker
):
    existing_review = mocker.Mock()
    ...
    query = db_session.query.return_value
    review_query = query.filter.return_value
    review_query.first.return_value = existing_review

    review_query.update.side_effect = database_error(
...
def test_delete_review_success(
    ...
    review_query.delete.assert_called_once_with(
        synchronize_session=False
    )
    db_session.commit.assert_called_once()
...
def test_delete_review_not_found(
    ...
    review_query.delete.assert_not_called()
    db_session.commit.assert_not_called()


def test_delete_review_database_error(
    db_session,
    mocker
):
    existing_review = mocker.Mock()
    ...
    review_query.delete.side_effect = database_error(
